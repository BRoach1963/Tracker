...
1. **Extend VectorStore** for data types - 2 hours
2. **Create DataIndexer service** - 2 hours
3. **Index team members** - 1 hour
4. **Index meetings & tasks** - 1 hour
5. **Test semantic search** - 1 hour
...
├── AI/VectorStore.cs (add data collections)
```
...
- **Embeddings**: ~1.5KB per entity × 1000 entities = ~1.5MB
- **Vector DB**: Uses in-memory SQLite (existing)
- **Indexing time**: ~30 seconds for 1000 entities
