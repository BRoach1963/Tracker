...
**Purpose:** Supabase Postgres + pgvector-backed vector store for semantic search.

**Storage Location:** Supabase-managed Postgres database (pgvector-enabled schema)
...
CREATE INDEX idx_ai_embeddings_tenant_entity
...
**Embedding Storage:**
- Format: pgvector column storing 768-dim float vectors
- Dimensions: 768 (text-embedding-004)
- Compression: Handled by Postgres/pgvector

**Search Algorithm:**
- Type: pgvector cosine similarity with IVF index (suitable for large-scale search)
- Optimization: Filter by tenant and metadata before vector search
- Future: Tune IVF lists/probes per environment
...
| Data | Typical Size |
|------|--------------|
| Supabase pgvector index | 5-50 MB (managed in Postgres) |
| ai_usage.json | <1 KB |
